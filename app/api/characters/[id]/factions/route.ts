...
export async function POST(
  ...
  try {
    const { factionId, role } = await req.json();
    if (!factionId)
      return NextResponse.json(
        { error: "factionId is required" },
        { status: 400 },
      );
    const result = await characterService.addFaction(params.id, factionId, role);
...
export async function DELETE(
  ...
  try {
    ...
    await characterService.removeFaction(params.id, factionId);
