...
export default function FactionDetailPage() {
  ...
  const handleAdd = async () => {
    const res = await apiFetch(`/api/factions/${id}/motivations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(form),
    });
    if (res.ok) {
      addToast({ title: "Motivation added" });
      setAdding(false);
      fetchFaction();
    } else {
  ...
  const handleRemove = async (motivationId: string) => {
    ...
    if (res.ok) {
      addToast({ title: "Motivation removed" });
      fetchFaction();
    }
  };
    ...
    <div className="space-y-6">
      ...
      {faction.description && (
        <Card>
          ...
          <CardContent>
            <p className="text-sm whitespace-pre-wrap">{faction.description}</p>
          </CardContent>
        </Card>
      )}
